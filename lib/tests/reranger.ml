...
let is_double_click =
  ...
  fun () ->
    ...
    fun () ->
      ...
      let result = let (k, t) = !last in k = k' && t +. 0.4 >= time in
      last := (k', time);
      result
...
let rec dir ?(initial_path=[]) ?after_width:(wref=ref 0) path =
  ...
  let rec render_directory ?(highlight=false) cell name =
    if highlight then (
      begin match !highlighted_cell with
      | None -> ()
      | Some (cell, name) -> render_directory cell name
      end;
      highlighted_cell := Some (cell, name);
    );
    Lwd_table.set cell @@ Ui.mouse_area
      (fun ~x:_ ~y:_ -> function
         | `Left ->
           render_directory ~highlight:true cell name;
           goto name; `Handled
         | _ -> `Unhandled)
      (string ~attr:Notty.(A.bg (if highlight then A.lightblue else A.blue)) name)
  in
  let add_directory name =
    if name <> "" && name.[0] <> '.' then
      let highlight = match initial_path with
  ...
  let add_file name =
    ...
    Lwd_table.set (Lwd_table.append files) @@ Ui.mouse_area
      (fun ~x:_ ~y:_ -> function
         | `Left ->
           if is_double_click () then
             ignore (Sys.command ("xdg-open " ^ Filename.quote (Filename.concat path name)) : int);
           `Handled
         | _ -> `Unhandled
      ) (string name)
  in
  ...
  Array.iter (fun name ->
      ...
      try
        ...
        then add_directory name
        else add_file name
      with exn ->
        let text = match exn with
          | Sys_error _ -> name
          | exn -> Printexc.to_string exn
        in
        Lwd_table.append' files (string ~attr:Notty.(A.bg A.red) text)
    ) entries;
  begin match initial_path with
  | [] -> ()
  | x :: xs -> goto ~initial_path:xs x
  end;
...
let () =
  let initial_path =
    let rec split path =
      ...
      if parent = path then []
      else Filename.basename path :: split parent in
    List.rev (split (Sys.getcwd ()))
  in
  Ui_loop.run (Lwd.map' (dir ~initial_path "/") (fun ui ->
