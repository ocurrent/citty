...
let ui =
  let place_ui_var ?sw v = Lwd.(v |> get |> join |> map (Ui.resize ~w:0 ?sw)) in
  let spacer = Ui.empty |> Ui.resize ~w:1 ~bg:Notty.A.(bg (gray 1)) |> Lwd.pure in
...
let with_ref r fn =
  Lwt.finalize
    (fun () -> fn r)
    (fun () -> C.Capability.dec_ref r; Lwt.return_unit)

let show_log table job =
  let add str =
    if str <> "" then
      Lwd_table.append' table str
  in
  let rec aux start =
...
let rec show_status var job =
  ...
  in
  Lwd.set buttons (Lwd_utils.pure_pack Ui.pack_x [
      rebuild_button;
      Ui.atom (Notty.I.void 1 0);
      cancel_button
    ]);
  ...
  let scrollbar = Lwd.get scroll_state |> Lwd.map (fun st ->
      ...
      if st.W.visible = 0 then
        ...
        let repeat f =
          ...
          let rec step delay () =
            if not !stop
            then (f (); Lwt_unix.sleep delay >>= step 0.025)
        ...
        Ui.mouse_area
             ...
             | `Left ->
               if y < prefix then
                 ...
                 repeat (fun () ->
                     ...
                     set_scroll `Change {st with position = !position};
                   )
                 ...
                 `Grab ((fun ~x:_ ~y:y' ->
                     ...
                     let position =
                       float st.position +.
                       float dy /. float st.W.visible *. float st.W.total
                     in
          ...
          (Ui.atom @@ Notty.I.vcat [
              bar gray prefix;
              bar lightgray ratio;
              bar gray suffix;
            ])
      else
        (Ui.atom (bar gray st.W.visible))
    )
  in
  let result =
    Lwd_utils.pack Ui.pack_y [
      ...
      Lwd_utils.pack Ui.pack_x [
        Lwd.get buttons;
        Lwd.pure (Ui.resize ~h:1 ~sw:1 ~bg:Notty.A.(bg (gray 1)) Ui.empty);
      ];
      Lwd_utils.pack Ui.pack_x [
        Lwd_utils.pack Ui.pack_y [
...
let select_next list =
  let rec seek = function
    ...
    | ((x, _), _) :: (item, _) :: _ when Lwd.peek x ->
      do_highlight item; true
    | _ :: rest -> seek rest
  in
  if seek list then ()
  else match list with
    | (item, _) :: _ -> do_highlight item
    | [] -> ()
